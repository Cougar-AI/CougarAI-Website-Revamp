...
- ✅ Guest QR check-in bugfixes — `ProtectedRoute` now calls `setPendingCheckinCode` itself (reading `?code=` off `location.search`) before redirecting a logged-out or not-yet-onboarded visitor away from `/checkin`, since `CheckIn.tsx` never mounts (and thus never captures the code) for a logged-out scan; `CheckIn.tsx` now clears the pending code in a `finally` block on both success AND failure so a failed check-in no longer leaves a stale code that hijacks a later, unrelated login's post-login redirect.
- ✅ Guest QR check-in login-hijack fix — closed a remaining hole where a stale pending code (scan abandoned before `CheckIn.tsx` ever mounted, so its `finally` clear never ran) could hijack a later, unrelated login: `Login.tsx`'s `resolvePostLoginDestination` now redirects to `/checkin` only when router `state.from` starts with `/checkin` and no longer reads/consumes the stored pending code at all, leaving consumption solely to `Onboarding.handleFinish` (new-user signup chain) and `CheckIn.tsx`; `lib/auth.ts`'s `setPendingCheckinCode`/`consumePendingCheckinCode` now store a `{code, storedAt}` timestamp and treat entries older than 2 hours (`PENDING_CHECKIN_CODE_TTL_MS`) as expired, as defense-in-depth against any residual staleness.
